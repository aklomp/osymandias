...
// A composable loop header that loops over all valid quads:
#define FOREACH_QUAD(node)			\
	for (int x = 0; x < 2; x++)		\
		for (int y = 0; y < 2; y++)	\
			if ((node)->q[x][y])
...
node_destroy_recursive (struct quadtree *t, struct node *n)
{
	...
	FOREACH_QUAD(n)
		node_destroy_recursive(t, n->q[x][y]);
...
purge_below (struct quadtree *t, struct node *n, int zoom, int prune_target)
{
	...
	FOREACH_QUAD(n) {
		purge_below(t, n->q[x][y], zoom, prune_target);
		if (t->num_data <= prune_target)
			return;
	}
}
...
purge_below_visibility (struct quadtree *t, struct node *n, int prune_target)
{
	...
	FOREACH_QUAD(n) {
		purge_below_visibility(t, n->q[x][y], prune_target);
		if (t->num_data <= prune_target)
			return;
	}
}
